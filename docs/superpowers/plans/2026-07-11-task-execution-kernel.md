...
- Pro allowlist 当前仅 QQ `<configured-qq-id>`，聊天、人格和记忆不能提升权限。
...
def test_approval_cannot_cross_task_or_step():
    pending = registry.issue("<configured-qq-id>", "private", "job123", "digest-a", "发送报告", "claude", WORK_DIR)
    assert registry.consume(pending.token, "<configured-qq-id>", "private", "job124", "digest-a") is None
    assert registry.consume(pending.token, "<configured-qq-id>", "private", "job123", "digest-b") is None
```
