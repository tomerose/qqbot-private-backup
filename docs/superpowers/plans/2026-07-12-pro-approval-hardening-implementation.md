...
- 公开 Pro 不得获得本机 Agent 权限；审核人与可信 Agent 用户只允许 `<configured-qq-id>`。
...
- [ ] Step 3: 重启 `<local-project-root>\qqbot\astrbot` 中的 `python -m astrbot.cli run`，验证 6185/6199 为回环监听、QQ WebSocket 已连通且无插件异常。
