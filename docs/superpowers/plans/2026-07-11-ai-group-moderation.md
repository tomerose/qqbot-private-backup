...
- 仅 QQ `<configured-qq-id>` 可开关或查询 AI 群管。
...
def test_context_removes_identifiers_paths_queries_and_secrets():
    text = sanitize_message("QQ<configured-qq-id> C:\\Users\\liu token=abc https://x.test/a?q=private")
    assert "<configured-qq-id>" not in text and "liu" not in text and "private" not in text
```
...
def test_store_never_contains_raw_ids_or_message_text():
    ...
    store.set_enabled("<configured-qq-id>", True)
    store.record_action("<configured-qq-id>", "123456789", "recall", "repeated_spam", 0.96, True, now=1000)
    raw = db.read_bytes()
    assert b"<configured-qq-id>" not in raw and b"123456789" not in raw
...
class FakeEvent:
    ...
    def get_group_id(self): return "<configured-qq-id>"
...
async def test_valid_decision_recalls_then_uses_local_mute_duration():
    ...
    assert bot.calls[1] == ("set_group_ban", <configured-qq-id>, 123456789, 60)
...
- Adds commands: `/AI群管开`, `/AI群管关`, `/AI群管状态`，仅 `OWNER_ID="<configured-qq-id>"`。
...
def test_only_owner_can_toggle_ai_moderation():
    ...
    assert 'OWNER_ID = "<configured-qq-id>"' in source
