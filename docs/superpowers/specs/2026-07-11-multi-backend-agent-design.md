...
仅 QQ `<configured-qq-id>` 可在私聊或群聊明确 @ 小柠后，调用本机 Claude Code、Codex CLI、WorkBuddy CLI 完成任务。Claude 为默认后端，现有全局 Claude Code 与 cc-connect 配置保持不变。
...
- 只有精确发送者 ID `<configured-qq-id>` 有执行权限。
- 群聊必须真实 @ 当前机器人。
