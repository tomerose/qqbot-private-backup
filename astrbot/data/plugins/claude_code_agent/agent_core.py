...
DEFAULT_WORKSPACE = Path(
    os.environ.get("XIAONING_WORKSPACE", str(Path.cwd() / "claude_workspace"))
).expanduser()
DEFAULT_WORK_DIR = DEFAULT_WORKSPACE.parent


def _tool_path(env_name: str, command: str) -> Path:
    configured = os.environ.get(env_name, "").strip()
    return Path(configured).expanduser() if configured else Path(shutil.which(command) or command)


CLAUDE_EXE = _tool_path("CLAUDE_CODE_BIN", "claude")
NODE_EXE = _tool_path("NODE_BIN", "node")
CODEX_CLI = _tool_path("CODEX_CLI", "codex")
WORKBUDDY_CLI = _tool_path("WORKBUDDY_CLI", "codebuddy")
