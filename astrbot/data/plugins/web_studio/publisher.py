...
FIREBASE_PROJECT = os.getenv("WEB_STUDIO_PROJECT", "").strip()
FIREBASE_SITE = os.getenv("WEB_STUDIO_SITE", "").strip()
PUBLIC_ORIGIN = os.getenv("WEB_STUDIO_PUBLIC_ORIGIN", "").strip().rstrip("/")
...
class FirebasePublisher:
    ...
    def deploy(self, page_id: str | None = None, should_exist: bool = True) -> str:
        if not FIREBASE_PROJECT or not PUBLIC_ORIGIN:
            raise PublishError("网页公开发布未配置，请先设置 WEB_STUDIO_PROJECT 和 WEB_STUDIO_PUBLIC_ORIGIN")
