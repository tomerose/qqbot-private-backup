...
class FriendCore(Star):
    ...
    def __init__(self, context: Context, config: dict | None = None):
        ...
        self._gift_admin_qq = str(self.config.get("gift_admin_qq", os.getenv("XIAONING_GIFT_ADMIN_QQ", ""))).strip()
    ...
    _ENGAGED_GROUP_HELP = {
        item.strip(): {"cooldown": 120, "min_confidence": 0.55}
        for item in os.getenv("XIAONING_ENGAGED_GROUP_IDS", "").split(",")
        if item.strip()
    }
    _FREQUENT_HELP_USERS = frozenset(
        item.strip()
        for item in os.getenv("XIAONING_FREQUENT_HELP_USERS", "").split(",")
        if item.strip()
    )
