...
_CONTACT_EMAIL = os.getenv("PUBLIC_CONTACT_EMAIL", "").strip()
CONTACT_REPLY = (
    f"联系邮箱：{_CONTACT_EMAIL}。邮件里说明用途和想咨询的内容即可。"
    if _CONTACT_EMAIL
    else "目前没有配置公开联系邮箱；你可以直接在当前对话说明想咨询的内容。"
)
...
_SHEN_GROUP_IDS = frozenset(
    item.strip()
    for item in os.getenv("XIAONING_SHEN_GROUP_IDS", "").split(",")
    if item.strip()
)  # 只由部署者本地配置，不写入公开代码
