...
class QQAdminPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        try:
            ...
            self.ai_moderation_store = AIModerationStore(
                private_dir / "moderation.db",
                private_dir / "audit_salt.bin",
                default_enabled=bool(ai_cfg.get("enabled", False)),
            )
            ...
            logger.error("[AIGroupMod] privacy_store_initialization_failed")
    ...
    @staticmethod
    def _is_ai_moderation_owner(event: AiocqhttpMessageEvent) -> bool:
        return str(event.get_sender_id() or "") == OWNER_ID
    ...
    async def _set_ai_moderation(self, event: AiocqhttpMessageEvent, enabled: bool):
        ...
        if event.is_private_chat():
            await event.send(event.plain_result("请在目标群内设置 AI 群管。"))
            event.stop_event()
            return
        if self.ai_moderation_store is None:
            await event.send(event.plain_result("隐私保护组件不可用，AI 群管保持关闭。"))
            event.stop_event()
            return
        self.ai_moderation_store.set_enabled(str(event.get_group_id()), enabled)
        await event.send(event.plain_result("AI 群管已开启。" if enabled else "AI 群管已关闭。"))
        event.stop_event()

    @filter.command("AI群管开")
    async def enable_ai_moderation(self, event: AiocqhttpMessageEvent):
        await self._set_ai_moderation(event, True)

    @filter.command("AI群管关")
    async def disable_ai_moderation(self, event: AiocqhttpMessageEvent):
        await self._set_ai_moderation(event, False)
    ...
    async def ai_moderation_status(self, event: AiocqhttpMessageEvent):
        if not self._is_ai_moderation_owner(event):
            ...
            enabled = self.ai_moderation_store.is_enabled(str(event.get_group_id()))
            await event.send(event.plain_result("AI 群管：已开启" if enabled else "AI 群管：已关闭"))
        event.stop_event()

    @filter.platform_adapter_type(filter.PlatformAdapterType.AIOCQHTTP)
    @filter.event_message_type(EventMessageType.GROUP_MESSAGE)
    async def ai_moderate(self, event: AiocqhttpMessageEvent):
        if self.ai_moderation is not None:
    ...
    async def terminate(self):
        ...
        if self.ai_moderation is not None:
            await self.ai_moderation.terminate()
        await self.db.close()
