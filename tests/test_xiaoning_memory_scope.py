...
class XiaoningMemoryScopeTests(unittest.TestCase):
    ...
    def test_firestore_outbox_stops_same_user_after_first_failure(self):
        first = SimpleNamespace(
            ...
            payload={"user_scope": "900000001", "value": "记忆"},
        )
        second = SimpleNamespace(
            event_id="event-2",
            operation="delete_all",
            aggregate_id="all",
            payload={"user_scope": "900000001"},
        )
    ...
    def test_legacy_agent_completion_wrapper_supplies_evidence(self):
        with patch.object(memory_module, "track_runtime_task_status") as tracker:
            memory_module.track_agent_task_complete("900000001", "生成报告")

        args = tracker.call_args.args
        kwargs = tracker.call_args.kwargs
        self.assertEqual(args[0], "900000001")
