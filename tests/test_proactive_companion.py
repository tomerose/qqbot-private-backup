...
class CompositeGroupSessionProbe(ConfigMixin, SessionMixin):
    def __init__(self):
        self.config = {
            ...
            "group_settings": {"enable": True, "session_list": ["900000002"]},
        }
        self.session_override_manager = None
        self.session_data = {}
...
class SchedulerProbe(SchedulerMixin):
    def __init__(self, message_count, *, idle_minutes=10):
        ...
        self.session_data = {
            "bot:GroupMessage:900000002": {
                "group_user_messages_since_proactive": message_count
            }
        }
        self.group_timers = {"bot:GroupMessage:900000002": TimerProbe()}
...
class GroupEvent:
    unified_msg_origin = "bot:GroupMessage:900000002"
...
class ProactiveCompanionTests(unittest.TestCase):
    def test_group_threshold_reply_skips_private_manual_authorization_gate(self):
        async def scenario():
            ...
            await probe.check_and_chat("bot:GroupMessage:900000002")
    ...
    def test_sender_scoped_group_umo_resolves_to_configured_group_session(self):
        ...
        first_member = "llbot-test:GroupMessage:2998891949_900000002"
        second_member = "llbot-test:GroupMessage:3376903140_900000002"

        self.assertEqual(
            probe._normalize_session_id(first_member),
            "llbot-test:GroupMessage:900000002",
        )
        self.assertEqual(
    ...
    def test_all_x_pro_sessions_excludes_ordinary_users_and_keeps_group_whitelist(self):
        ...
        with patch(
            ...
            self.assertIsNotNone(probe._get_session_config("bot:FriendMessage:900000001"))
        with patch(
            "astrbot_plugin_proactive_chat.core.session_config.get_tier",
            return_value=Tier.ORDINARY,
        ):
            self.assertIsNone(probe._get_session_config("bot:FriendMessage:2000000000"))
        self.assertIsNotNone(probe._get_session_config("bot:GroupMessage:900000002"))
    ...
    def test_shared_cooldown_and_quiet_mode_stop_private_outreach(self):
        state = {}
        self.assertTrue(can_send_proactive(state, "900000001", 6 * 3600, now=100))
        record_proactive_send(state, "900000001", now=100)
        self.assertFalse(can_send_proactive(state, "900000001", 6 * 3600, now=200))
        self.assertTrue(can_send_proactive(state, "900000001", 6 * 3600, now=22000))
        set_friend_mode(state, "900000001", QUIET_MODE)
        self.assertFalse(can_send_proactive(state, "900000001", 0, now=22000))
    ...
    def test_proactive_memory_is_requested_only_for_private_context(self):
        ...
        async def scenario():
            ...
            self.assertIn(
                "memory:900000001",
                await probe._get_proactive_memory_block("bot:FriendMessage:900000001", history),
            )
            self.assertEqual(
                await probe._get_proactive_memory_block("bot:GroupMessage:900000002", history),
                "",
            )
    ...
    def test_group_hybrid_context_prefers_live_onebot_history(self):
        ...
        async def scenario():
            ...
            self.assertEqual(client.calls[0][1]["group_id"], 900000002)
    ...
    def test_group_silence_requires_a_member_message(self):
        async def scenario():
            session_id = "bot:GroupMessage:900000002"
    ...
    def test_all_private_mode_restores_known_private_sessions_on_startup(self):
        async def scenario():
            ...
            self.assertEqual(probe.setup_sessions, ["default:FriendMessage:900000001"])
