...
def load_router():
    ...
    def platform_adapter_type(*_args, **kwargs):
        def decorate(fn):
            fn._test_filter_priority = kwargs.get("priority", 0)
            return fn
        return decorate
...
class ChatRouterTests(unittest.TestCase):
    ...
    def test_shutdown_gate_blocks_every_group_path(self):
        module = load_router()
        router = module.ChatRouter(types.SimpleNamespace())
        ...
        self.assertTrue(urgent.stopped)

    def test_shutdown_gate_blocks_owner_and_non_owner_private_chat_first(self):
        module = load_router()
        router = module.ChatRouter(types.SimpleNamespace())
        ...
        for sender in ("900000001", "900000007"):
            event = Event(sender)
            asyncio.run(router.gate_group_reply(event))
            self.assertTrue(event.stopped)

        self.assertGreaterEqual(router.gate_group_reply._test_filter_priority, 10_000)
