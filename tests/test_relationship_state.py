...
class RelationshipStateTests(unittest.TestCase):
    def test_records_interaction_and_return_gap(self):
        state = {}
        record_interaction(state, "900000001", now=100)
        entry = record_interaction(state, "900000001", now=100 + 21 * 3600)

        self.assertEqual(entry["message_count"], 2)
        self.assertEqual(entry["last_return_gap_hours"], 21.0)
        self.assertEqual(get_snapshot(state, "900000001", now=100)["friend_mode"], NORMAL_MODE)
    ...
    def test_mode_parse_and_set_are_plain_state_not_permission(self):
        ...
        self.assertEqual(get_snapshot(state, "900000001")["friend_mode"], QUIET_MODE)
        self.assertNotIn("tier", state["900000001"])
