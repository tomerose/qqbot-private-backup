...
class ConversationGuardTests(unittest.TestCase):
    ...
    def test_runtime_config_uses_event_driven_private_followups_and_silent_groups(self):
        ...
        if set(config["plugin_set"]) == {"chat_router", "xiaoning_scheduled"}:
            self.assertNotIn("astrbot_plugin_proactive_chat", config["plugin_set"])
            self.assertTrue(config.get("disable_builtin_commands"))
            self.assertFalse(config.get("platform_settings", {}).get("weixin_oc", {}).get("enable", False))
            return
        ...
        self.assertIn("连续短句合并理解", persona)
        self.assertIn("前后说法冲突时纠正具体错误", persona)
        ...
        self.assertEqual(proactive["friend_settings"]["session_list"], ["default:FriendMessage:900000001"])
