    ...
    def __init__(
        ...
        *,
        ...
        self.unified_msg_origin = (
            ...
            if group_id
            else "aiocqhttp:FriendMessage:900000001"
        )
        self._extra = {}
        self._result = None
        self.sent = []
...
class AgentIntegrationTests(unittest.TestCase):
        ...
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                ...
                await plugin._track_cross_dialog_job(
                    "900000001", "abc123", "制作报告", "delivery_pending", "not delivered"
                )
                self.assertEqual(
                    calls,
                    [("900000001", "abc123", "制作报告", "delivery_pending", "not delivered")],
    ...
    def test_delivery_pending_survives_restart_for_queue_worker(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            plugin._job_store.start(
                job_id,
                "900000001",
                "aiocqhttp:FriendMessage:900000001",
                "生成报告",
                "claude",
                "",
            ...
            with tempfile.TemporaryDirectory() as tmp:
                ...
                with patch.object(file_cache, "_CACHE_PATH", cache_path):
                    file_cache.record_file(
                        "生成报告", str(cached), sender_id="900000001", job_id="old"
                    )
    ...
    def _plugin(self, root: Path):
        ...
        plugin.config = {}
        plugin._access_policy = AccessPolicy(["900000001"])
        plugin._trusted_policy = TrustedPolicy(["900000001"])
    ...
    def test_x_member_can_execute_natural_task(self):
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                plugin = self._plugin(Path(tmp))
                plugin._access_policy = AccessPolicy(["900000001", "2000000000"])
    ...
    def test_restart_resumes_safe_job_and_blocks_non_replayable_job(self):
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                ...
                plugin._job_store.start(
                    safe_id, "900000001", "aiocqhttp:FriendMessage:900000001", "读取项目并生成报告",
                    "claude", "", state="running", recovery="replay_safe"
                )
                plugin._job_store.start(
                    blocked_id, "900000001", "aiocqhttp:FriendMessage:900000001", "删除旧目录",
                    "claude", "删除", state="running", recovery="blocked"
                )
                plugin._payload_store.write(
                    safe_id,
                    EncryptedJobPayload(
                        "读取项目并生成报告", "aiocqhttp:FriendMessage:900000001", "claude", "project", "replay_safe"
                    ),
                )
                plugin._payload_store.write(
                    blocked_id,
                    EncryptedJobPayload(
                        "删除旧目录", "aiocqhttp:FriendMessage:900000001", "claude", ".", "blocked"
                    ),
                )
        ...
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                ...
                plugin._payload_store.write(
                    ...
                    EncryptedJobPayload(
                        task=task,
                        scope="aiocqhttp:FriendMessage:900000001",
                        backend="claude",
        ...
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                ...
                plugin._job_store.start(
                    job_id,
                    "900000001",
                    "llbot-test:FriendMessage:900000001",
                    "生成两个报告",
                    "claude",
                    "",
                    state="running",
                ...
                self.assertEqual(kwargs["user_id"], 900000001)
    ...
    def test_group_upload_failure_falls_back_to_requester_private_chat(self):
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                plugin = self._plugin(Path(tmp))
                event = FakeEvent(
                    "帮我生成 report.docx", group_id="900000002", at_self=True
                )
                ...
                self.assertEqual(event.bot.actions[-1][1]["user_id"], 900000001)
    ...
    def test_recovered_group_file_uses_onebot_group_upload(self):
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertEqual(kwargs["group_id"], 900000002)
    ...
    def test_replayed_group_job_uses_onebot_for_new_file(self):
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                ...
                job_id = "c1d2e3f4a5b6"
                job_dir = create_job_dir(plugin.workspace, job_id)
                scope = "llbot-test:GroupMessage:900000002"
                plugin._job_store.start(
                    job_id,
                    "900000001",
                    scope,
