...
from xiaoning_runtime import ArtifactDeliveryResult  # noqa: E402
...
class FakeEvent:
    ...
    def get_messages(self):
        return getattr(self, "message_chain", [])
...
class DrawPluginTests(unittest.TestCase):
    def setUp(self):
        ...
        task_mirror = patch.object(
            draw_main, "mirror_runtime_task_status", new=AsyncMock()
        )
        task_mirror.start()
    ...
    def test_qq_cdn_reference_image_is_loaded_for_editing(self):
        image = PillowImage.new("RGB", (2, 2), "white")
        buffer = io.BytesIO()
        ...
        class Segment:
            type = "image"
            file = "reference.png"
            url = "https://multimedia.nt.qq.com.cn/download?fileid=test"

        class MessageObject:
            message = [Segment()]

        event = FakeEvent("重画", "900000001")
        event.message_obj = MessageObject()
        with patch.object(draw_main.requests, "get", return_value=Response()) as get:
            encoded = DrawCommand._get_referenced_image_base64(event)

        self.assertTrue(encoded)
        get.assert_called_once()

    def test_redraw_without_reference_is_stopped_before_normal_chat(self):
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertEqual(
                    replies,
                    [("plain", "已记住修改要求。请把原图发来，或回复那张图；收到图片后才会开始处理。")],
                )
    ...
    def test_generic_edit_intent_first_then_image_still_executes(self):
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                ...
                image_event = FakeEvent("", "900000001")
                with patch.object(plugin, "_get_referenced_image_base64", return_value=encoded):
    ...
    def test_group_edit_session_is_isolated_by_sender(self):
        first = FakeEvent("", "2000000001", private=False, wake=True)
        second = FakeEvent("", "2000000002", private=False, wake=True)
        first.unified_msg_origin = second.unified_msg_origin = "llbot:GroupMessage:900000002"
        self.assertNotEqual(DrawCommand._edit_scope(first), DrawCommand._edit_scope(second))
    ...
    def build_plugin(output_root: Path) -> DrawCommand:
        ...
        ProStore(db_path, reviewer_id="900000001")
        ...
        plugin._daily_usage = {}
        plugin._edit_sessions = ImageEditSessionStore(output_root.parent / "edit_sessions")
        return plugin

    @staticmethod
    def activate_pro(db_path: Path, qq_id: str):
        store = ProStore(db_path, reviewer_id="900000001")
        now = time.time()
        app = store.create_application(qq_id, now=now)
        store.mark_sent(app.application_id, qq_id, now=now + 1)
        store.request_approval(app.application_id, "900000001", 90, now=now + 2)
        code = store.confirm_approval(app.application_id, "900000001", now=now + 3)
    ...
    def test_revoked_member_falls_back_to_ordinary_draw_allowance(self):
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                ...
                plugin = self.build_plugin(root)
                self.activate_pro(plugin._pro_db_path, "2000000000")
                store = ProStore(plugin._pro_db_path, reviewer_id="900000001")
                self.assertTrue(store.revoke("2000000000", "900000001", now=time.time()))
                image = PillowImage.new("RGB", (2, 2), "blue")
                buffer = io.BytesIO()
                image.save(buffer, format="PNG")
                plugin._request_image = lambda *_args: buffer.getvalue()
    ...
    def test_stale_dewatermark_session_does_not_crash_confirmation(self):
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                plugin = self.build_plugin(Path(tmp) / "draws")
                scope = DrawCommand._edit_scope(FakeEvent("确认", "900000001"))
                ...
                replies = await collect(plugin.on_message(FakeEvent("确认", "900000001")))
    ...
    def test_generation_failure_returns_without_delivery_crash(self):
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                ...
                plugin._request_image = fail_request
                replies = await collect(plugin.on_message(FakeEvent("/draw a cat", "2000000000")))

                self.assertEqual(replies[-1], ("plain", "画图没成功，我换个方式帮你。"))
    ...
    def test_group_draw_without_at_mention_is_ignored(self):
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                plugin = self.build_plugin(Path(tmp))
                event = FakeEvent("/draw a cat", "900000001", private=False, wake=False)
    ...
    def test_generated_image_is_sanitized_to_private_png_and_cleanup_keeps_outside_file(self):
        async def scenario():
            with tempfile.TemporaryDirectory() as tmp:
                ...
                plugin = self.build_plugin(root)
                plugin._request_image = lambda *_args: buffer.getvalue()
                event = FakeEvent("/draw a cat", "900000001")
