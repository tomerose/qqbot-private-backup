...
class AgentAccessPolicyTests(unittest.TestCase):
    ...
    def test_pro_allowlist_is_frozen_and_grants_agent_capabilities(self):
        configured = ["900000001"]
        policy = AccessPolicy(configured)
        configured.append("2000000000")

        self.assertEqual(policy.resolve_tier("900000001"), AccessTier.PRO)
        for capability in Capability:
            self.assertTrue(policy.authorize("900000001", capability))
        self.assertEqual(policy.resolve_tier("2000000000"), AccessTier.ORDINARY)

    def test_invalid_ids_and_chat_phrases_never_become_pro(self):
        policy = AccessPolicy(["900000001", "领取Pro", "", "not-a-qq"])

        self.assertEqual(policy.pro_user_ids, frozenset({"900000001"}))
    ...
    def test_approved_public_pro_cannot_become_trusted_host_operator(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = ProStore(Path(tmp) / "pro_members.db", reviewer_id="900000001")
            application = store.create_application("2000000000", now=1_000)
            store.mark_sent(application.application_id, "2000000000", now=1_001)
            store.request_approval(application.application_id, "900000001", 90, now=1_002)
            code = store.confirm_approval(application.application_id, "900000001", now=1_003)
            store.verify("2000000000", code, now=1_004)
            self.assertTrue(store.is_active_pro("2000000000", now=1_005))

        policy = AccessPolicy(["900000001", "2000000000"])
        trusted = TrustedPolicy(["900000001"])
        ...
        self.assertTrue(trusted.is_trusted("900000001"))

    def test_signed_public_pro_is_admitted_to_the_same_hard_agent_policy(self):
        agent = ClaudeCodeAgent.__new__(ClaudeCodeAgent)
        agent._access_policy = AccessPolicy(["900000001"])
        agent._trusted_policy = TrustedPolicy(["900000001"])
        ...
        owner_context = type("OwnerContext", (), {"get_sender_id": lambda self: "900000001"})()
