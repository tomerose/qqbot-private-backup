...
class AgentCoreTests(unittest.TestCase):
    ...
    def test_redact_local_paths_removes_owner_details_from_reply(self):
        text = r"文件在 `D:\public-project\qqbot\astrbot\data\workspaces\llbot-test_GroupMessage_900000002\报告.docx`，已经完成。"
    ...
    def test_sensitive_text_redacts_paths_and_credentials(self):
        raw = (
            r"文件在 C:\Users\test-user\secret.txt，"
            "Authorization: Bearer abcdefghijklmnopqrstuvwxyz "
            "api_key=sk-abcdefghijklmnopqrstuvwxyz123456"
        )
        cleaned = redact_sensitive_text(raw)
        self.assertNotIn(r"C:\Users\test-user", cleaned)
    ...
    def test_approval_is_one_time_expiring_and_bound_to_owner_and_chat(self):
        ...
        self.assertIsNone(registry.consume(pending.token, "900000001", "private", now=1001))
        approved = registry.consume(pending.token, "900000001", "group:123", now=1001)
        self.assertEqual(approved.task, "删除旧目录")
        self.assertIsNone(registry.consume(pending.token, "900000001", "group:123", now=1002))

        expired = registry.issue("900000001", "group:123", "安装软件", "codex", Path(r"D:\work"), now=2000)
        self.assertIsNone(registry.consume(expired.token, "900000001", "group:123", now=2301))

    def test_latest_approval_is_scoped_one_time_and_ignores_expired_entries(self):
        registry = ApprovalRegistry(ttl_seconds=300)
        registry.issue(
            "900000001", "group:123", "删除 A", "claude", Path(r"D:\work"),
            now=1000, task_id="job1", step_digest="a" * 64,
        )
        newest = registry.issue(
            "900000001", "group:123", "删除 B", "codex", Path(r"D:\work"),
            now=1010, task_id="job2", step_digest="b" * 64,
        )
        registry.issue(
            "900000001", "group:999", "删除 C", "claude", Path(r"D:\work"),
            now=1020, task_id="job3", step_digest="c" * 64,
        )

        # No-task filter → multiple candidates → ambiguity guard returns None.
        self.assertIsNone(registry.consume_latest("900000001", "group:123", now=1021))
        # Explicit task_id filter → single match → consumed.
        approved = registry.consume_latest(
            "900000001", "group:123", now=1021,
        ...
        older = registry.consume_latest("900000001", "group:123", now=1022)
        self.assertIsNotNone(older)
        self.assertEqual(older.task, "删除 A")
        # Expired: nothing left.
        self.assertIsNone(registry.consume_latest("900000001", "group:123", now=1401))

    def test_latest_approval_works_with_single_candidate_no_filter(self):
        registry = ApprovalRegistry(ttl_seconds=300)
        registry.issue("900000001", "group:123", "删除 A", "claude", Path(r"D:\work"),
                        now=1000, task_id="job1", step_digest="a" * 64)
        # Single candidate → no-filter OK.
        approved = registry.consume_latest("900000001", "group:123", now=1001)
        self.assertIsNotNone(approved)
    ...
    def test_group_file_delivery_uses_onebot_upload_action(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            bot = FakeBot()
            asyncio.run(upload_aiocqhttp_group_file(bot, "900000002", path))
            self.assertEqual(len(bot.calls), 1)
            self.assertEqual(bot.calls[0][0], "upload_group_file")
            self.assertEqual(bot.calls[0][1]["group_id"], 900000002)
    ...
    def test_private_file_delivery_uses_onebot_upload_action(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path.write_bytes(b"pdf")
            bot = FakeBot()
            asyncio.run(upload_aiocqhttp_private_file(bot, "900000001", path))
            self.assertEqual(len(bot.calls), 1)
            self.assertEqual(bot.calls[0][0], "upload_private_file")
            self.assertEqual(bot.calls[0][1]["user_id"], 900000001)
            self.assertEqual(bot.calls[0][1]["name"], "answer.pdf")
            self.assertEqual(
                bot.calls[0][1]["file"],
                str(path.resolve()),
